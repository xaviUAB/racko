...
export const renderGame = (game, userId, isMyTurn, getMyPlayer, getNextPlayerId) => {
    ...
    // Handle waiting for player data synchronization
    if (!myPlayer) {
        document.getElementById('game-active-area').classList.add('hidden');
        container.innerHTML = `
...
const updateGameStatus = (game, userId) => {
...
const renderRack = (game, userId, isMyTurn, heldCard) => {
    const myPlayer = game.players.find(p => p.id === userId);
    if (!myPlayer) return;
...
const renderDrawPileAndDiscard = (game, isMyTurn, heldCard) => {
...
const renderActionArea = (game, userId, isMyTurn, heldCard, heldSource) => {
    const isFinishedHand = game.status === 'finishedhand';
    const isFinishedGame = game.status === 'finished';
...
const renderPlayerScores = (game, userId, isMyTurn) => {
